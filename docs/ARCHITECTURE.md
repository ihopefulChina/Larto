...
渲染层是一行窗口命令栏（地址、机型、缩放、预览/清缓存/调试器/设置/账号），下面左右对齐：左列模拟器画布，右列原生 DevTools。调试器打开时左右栏中间有官方同款 sash：左列默认 `max(410, deviceWidth × zoom + 32)`，PC 为 500；右列最小 300。拖动改左列宽度并写入 `settings.simulatorColumnWidth`，切机型回到自动宽度，双击 sash 也回到自动。左右栏之间是 10px 原生 sash 条（压在 guest 与 DevTools 之上）。按下即开拖，全窗口透明层按屏幕位移跟手，松开落盘。`.gadgetBox` 取缩放后的实际尺寸并 `margin: 0 auto` 居中（溢出时自动贴左、可滚动），内部 `.gadget` 保持 CSS 像素尺寸、`transform: scale()` 以左上角为原点。PC 机型默认 `.gadgetBox.pc` 填满列（`calc(100% - 40px)`），缩放时按同一比例缩小/放大视觉框；右下角 28×28 命中区按视觉 1:1（指针位移 ÷ zoom）拖成固定 viewport，拖动时用 latest-only 单飞队列发送 background CDP 预览，松开后才持久化一次。Esc 取消、方向键（10px；Shift 50px；Option 1px）和可编辑宽高输入提供非拖拽路径，「适应」恢复响应式布局。地址历史、机型与缩放菜单都向下展开。
