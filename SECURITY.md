...
- MCP listens only on `127.0.0.1`. It intentionally has no separate authentication layer and can
  ...
  forward its port.
- Version 0.1.5 is an early release: the `.app` inside each macOS DMG/ZIP is ad-hoc signed and not
...
[GitHub Releases](https://github.com/ihopefulChina/Larto/releases). The 0.1.5 release
...
```bash
sha256sum Larto-0.1.5-linux-x64.AppImage
gh attestation verify Larto-0.1.5-mac-arm64.dmg \
