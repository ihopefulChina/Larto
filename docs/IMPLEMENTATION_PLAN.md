...
验收：`v0.1.1` Release 中各平台资产与 SHA-256 清单齐全，Actions 原生 smoke 通过，官网 Pages 已部署；`npm view larto-mcp@0.1.1 version` 可查。任何一项未完成都不能写成已发布。当前桌面与 MCP 包版本为 **0.1.6**（分栏拖拽不再在机身上留竖线，松手后不再跟光标走）；发布结果以 `docs/progress.md` 最新记录为准。
