...
| --- | --- |
| 0.1.7 | Supported |
| 0.1.6 and earlier | Not supported; upgrade to the latest release |
...
- MCP listens only on `127.0.0.1`. It intentionally has no separate authentication layer and can
  ...
  forward its port.
- Version 0.1.7 is an early release: the `.app` inside each macOS DMG/ZIP is ad-hoc signed and not
...
[GitHub Releases](https://github.com/ihopefulChina/Larto/releases). The 0.1.7 release
...
```bash
sha256sum Larto-0.1.7-linux-x64.AppImage
gh attestation verify Larto-0.1.7-mac-arm64.dmg \
