...
- 模拟器设备菜单与系统「视图 → 设备」菜单按 iPhone / Android / iPad / PC 分组，带分组标题，不再是一条没有分类的长列表。
- PC 模式下可拖拽左侧模拟器与 DevTools 之间的分栏，宽度写入设置；双击分栏恢复自动宽度。PC 画布的右下角缩放手柄移出 `<webview>` 命中区，避免原生 guest view 吞掉拖拽。
